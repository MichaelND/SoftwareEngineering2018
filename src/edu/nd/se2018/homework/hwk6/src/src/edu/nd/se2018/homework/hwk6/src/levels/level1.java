...
public class level1 implements levelStrategy{
	...
	public int[][] createMap(int chipCount, int[][] mapGrid, ObservableList<Node> root, int scale, int dimensions) {
		...
		// Randomly Spawn Key and set correct color wall
		...
		while (chipCount != 0) {
			...
			int chipX = randomChip.nextInt(15) + 5; // Generate random chip position
			int chipY = randomChip.nextInt(16) + 4;
			while (mapGrid[chipX][chipY] >= 2 && mapGrid[chipX][chipY] <= 6) {	  // Tile is already a chip or key
				chipX = randomChip.nextInt(15) + 5; // Generate random chip position
