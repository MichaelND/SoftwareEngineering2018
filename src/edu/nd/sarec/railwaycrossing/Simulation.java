...
/**
 * Simulation 
 * Edits: Added second train and added new junction
 */
...
public class Simulation extends Application{
	...
	public void start(Stage stage) throws Exception {
		...
		RailwayTracks track2 = mapBuilder.getTrack("Royal2"); //new train
		Train train = new Train(track.getEndX()+100,track.getEndY()-25);
		Train train2 = new Train(track2.getStartX()-100,track2.getEndY()-15);
		
		train.setMoveDirection("West");
		train2.setMoveDirection("East");
		...
		new AnimationTimer(){
			...
			public void handle(long now) {
				createCar();
				train.setEastBoundTrainX(train2.getVehicleX()); //store position of east bound train
				train.move();
				train2.setWestBoundTrainX(train.getVehicleX()); //store position of west bound train
				train2.move();
